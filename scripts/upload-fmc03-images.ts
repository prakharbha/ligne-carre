/**
 * Script to optimize and upload FMC03 portfolio images to Sanity
 * Run with: npx tsx scripts/upload-fmc03-images.ts
 */
...
async function uploadFMC03Images() {
  console.log('🌱 Starting FMC03 portfolio image upload to Sanity...\n');

  try {
    // Find the FMC03 portfolio item
    const portfolioItems = await client.fetch(
      `*[_type == "portfolioItem" && slug_en.current == "fmc03-heritage-private-residence-lot-669"]`
    );

    if (portfolioItems.length === 0) {
      console.error('❌ FMC03 portfolio item not found. Please run the seed script first.');
      process.exit(1);
    }
    ...
    const imageFiles = readdirSync(imageFolder).filter(file => 
      /\.(jpg|jpeg|JPG|JPEG|png|PNG)$/i.test(file)
    );
    ...
    console.log(`🎉 All ${imageFiles.length} images uploaded and linked to FMC03 portfolio item!`);
