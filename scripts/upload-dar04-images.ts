/**
 * Script to optimize and upload DAR04 portfolio images to Sanity
 * Run with: npx tsx scripts/upload-dar04-images.ts
 */
...
async function optimizeImage(inputPath: string, backupPath?: string): Promise<Buffer> {
  ...
  // Use quality 85 for better compression while maintaining good visual quality
  ...
  if (backupPath) {
    const backupDir = resolve(process.cwd(), 'backups/optimized-images/dar04');
...
async function uploadDAR04Images() {
  console.log('🌱 Starting DAR04 portfolio image upload to Sanity...\n');

  try {
    // Find the DAR04 portfolio item
    ...
    if (portfolioItems.length === 0) {
      console.error('❌ DAR04 portfolio item not found. Please run "npx tsx scripts/add-dar04-portfolio.ts" first.');
      process.exit(1);
    }
    ...
    // Get all images from the dar-04 folder
    const imageFolder = 'dar-04';
    ...
    const imageFiles = readdirSync(imageFolder)
      .filter(file => 
        /\.(jpg|jpeg|JPG|JPEG|png|PNG|tif|tiff|TIF|TIFF|bmp|BMP)$/i.test(file) && !file.startsWith('.')
      )
    ...
    const mainBackupPath = resolve(process.cwd(), 'backups/optimized-images/dar04', `main-${mainImageFile.replace(/\.(jpg|jpeg|JPG|JPEG|png|PNG|tif|tiff|TIF|TIFF|bmp|BMP)$/i, '.webp')}`);
    const mainOptimized = await optimizeImage(mainImagePath, mainBackupPath);
    const mainAsset = await uploadImage(mainOptimized, `dar04-main-${mainImageFile.replace(/\.(jpg|jpeg|JPG|JPEG|png|PNG|tif|tiff|TIF|TIFF|bmp|BMP)$/i, '.webp')}`);
    ...
    for (let i = 0; i < galleryImageFiles.length; i++) {
      ...
      const galleryBackupPath = resolve(process.cwd(), 'backups/optimized-images/dar04', `gallery-${i + 1}-${galleryFile.replace(/\.(jpg|jpeg|JPG|JPEG|png|PNG|tif|tiff|TIF|TIFF|bmp|BMP)$/i, '.webp')}`);
      const galleryOptimized = await optimizeImage(galleryImagePath, galleryBackupPath);
      const galleryAsset = await uploadImage(galleryOptimized, `dar04-gallery-${i + 1}-${galleryFile.replace(/\.(jpg|jpeg|JPG|JPEG|png|PNG|tif|tiff|TIF|TIFF|bmp|BMP)$/i, '.webp')}`);
    ...
    console.log(`🎉 All ${imageFiles.length} images uploaded and linked to DAR04 portfolio item!`);
    console.log(`   - Main image: 1`);
    console.log(`   - Gallery images: ${galleryImageFiles.length}`);
    console.log(`   - Backups saved to: backups/optimized-images/dar04/`);
  } catch (error: any) {
