...
async function uploadFMC02Images() {
  ...
  try {
    ...
    const mainAsset = await uploadImage(mainOptimized, `fmc02-main-${mainImageFile.replace(/\.(jpg|jpeg|JPG|JPEG|png|PNG|tif|tiff|TIF|TIFF)$/i, '.webp')}`);
    ...
    for (const galleryFile of galleryImageFiles) {
      ...
      const galleryAsset = await uploadImage(galleryOptimized, `fmc02-gallery-${galleryFile.replace(/\.(jpg|jpeg|JPG|JPEG|png|PNG|tif|tiff|TIF|TIFF)$/i, '.webp')}`);
